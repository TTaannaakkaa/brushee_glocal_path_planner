...
BrusheeLocalMapCreator::BrusheeLocalMapCreator():private_nh_("~")
{
    ...
    sub_obs_poses_ = nh_.subscribe("/obstacle_pose", 1, &BrusheeLocalMapCreator::obs_poses_callback, this);
    ...
    local_map_raw_.info.width = int(round(local_map_size_ / local_map_resolution_));
    local_map_raw_.info.height = int(round(local_map_size_ / local_map_resolution_));
...
void BrusheeLocalMapCreator::obs_poses_callback(const geometry_msgs::PoseArrayConstPtr& msg)
{
    obs_poses_ = *msg;
    ...
    for(int i=0;i<local_map_raw_.data.size();i++)
    {
        if(local_map_raw_.data[i] == 100)
        {
            ...
            for(int j=-3;j<3;j++)
            {
                for(int k=-3;k<3;k++)
                {
                    int index = (index_x + k) + ((index_y + j) * local_map_raw_.info.width);
...
bool BrusheeLocalMapCreator::is_in_local_map(const double dist, const double angle)
{
...
int BrusheeLocalMapCreator::get_grid_index(const double dist, const double angle)
{
    const double x = dist * cos(angle);
    const double y = dist * sin(angle);
